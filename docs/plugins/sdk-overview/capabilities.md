...
New implementations use `WorkerProvider<1>` from
`openclaw/plugin-sdk/plugin-entry` and declare `liveAuthorityVersion: 1`.
Core supplies a required, invocation-bound `assertCurrent()` to both
`prepareProvision` and `provision`, including the returned allocation function.
Call it before external effects and after awaited work before further effects or
durable writes. The guard rejects when the operation owner changes, destruction
is requested, the service stops, or the invocation closes (including timeout).
The same operation's node enrollment does not retire its provisioning guard.
Do not retain the guard for renewal or cleanup: cleanup has independent authority.
Version 1 also requires the existing SSH identity-resolution invocation guard.
An omitted version retains the legacy SDK contract and existing lifecycle and
placement support; unknown declarations are rejected. See
[worker-provider compatibility](/plugins/sdk-migration/compatibility-policy#worker-provider-invocation-guards).
